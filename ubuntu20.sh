...
#
for COUNTCLIENTS in {1..$_NUMBER_OF_CLIENTS}
do
...
Address = ${_BASE_NET_ADDRESS}.1/24
ListenPort = ${_WG_LISTEN_PORT}
PostUp = iptables -A FORWARD -i wg0 -j ACCEPT; iptables -t nat -A POSTROUTING -o ens3 -j MASQUERADE; ip6tables -A FORWARD -i wg0 -j ACCEPT; ip6tables -t nat -A POSTROUTING -o ens3 -j MASQUERADE; iptables -t nat -A POSTROUTING -s ${_BASE_NET_ADDRESS}.0/24 -o eth0 -j MASQUERADE
PostDown = iptables -D FORWARD -i wg0 -j ACCEPT; iptables -t nat -D POSTROUTING -o ens3 -j MASQUERADE; ip6tables -D FORWARD -i wg0 -j ACCEPT; ip6tables -t nat -D POSTROUTING -o ens3 -j MASQUERADE; iptables -t nat -D POSTROUTING -s ${_BASE_NET_ADDRESS}.0/24 -o eth0 -j MASQUERADE
SaveConfig = true 
...
sudo iptables -A INPUT -s ${_BASE_NET_ADDRESS}.0/24 -p udp -m udp --dport 53 -m conntrack --ctstate NEW -j ACCEPT
...
#-----------------------------------------------------------------------
# CREATING CLIENT FILES (to be copied to each client)

echo "### For each CLIENT"

export countFiles=0
for FILE in wg/keys/*client*private_key
do
    echo found ${FILE}
